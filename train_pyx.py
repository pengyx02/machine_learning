...
def show_loss_acc(history):
    ...
    plt.xlabel('epoch')
    plt.savefig('results/results_pyx.png', dpi=100)
...
def train(epochs):
    ...
    model.save("results/modle_pyx.h5")
    # 记录结束时间
    end_time = time()
...
def test_pyx():
    ...
    model = tf.keras.models.load_model("results/modle_pyx.h5")
    # model.summary()
    # 测试
    loss, accuracy = model.evaluate(test_ds)
    # 输出结果
    print('pyx test accuracy :', accuracy)
    ...
    show_heatmaps(title="heatmap", x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
                  save_name="results/heatmap_pyx.png")
